...
#include "amsstabilityr1.h"

namespace OHOS {
namespace AppExecFwk {
void AmsStAbilityR1::Init(const std::shared_ptr<AbilityInfo> &abilityInfo,
    const std::shared_ptr<OHOSApplication> &application, std::shared_ptr<AbilityHandler> &handler,
    const sptr<IRemoteObject> &token)
{
    HILOG_INFO("AmsStAbilityR1::Init");
...
void AmsStAbilityR1::OnStart(const Want &want)
{
    GetWantInfo(want);
    HILOG_INFO("AmsStAbilityR1::onStart");
    Ability::OnStart(want);
    std::string eventData = GetAbilityName() + STEventName::g_abilityStateOnStart;
    pageAbilityEvent.PublishEvent(STEventName::g_eventName, pageAbilityEvent.GetOnStartCount(), eventData);
}

void AmsStAbilityR1::OnNewWant(const Want &want)
{
    HILOG_INFO("AmsStAbilityR1::OnNewWant");
...
void AmsStAbilityR1::OnForeground(const Want &want)
{
    HILOG_INFO("AmsStAbilityR1::OnForeground");
...
void AmsStAbilityR1::OnStop()
{
    HILOG_INFO("AmsStAbilityR1::onStop");
    Ability::OnStop();
...
void AmsStAbilityR1::OnActive()
{
    HILOG_INFO("AmsStAbilityR1::OnActive");
    Ability::OnActive();
...
void AmsStAbilityR1::OnInactive()
{
    HILOG_INFO("AmsStAbilityR1::OnInactive");
    Ability::OnInactive();
...
void AmsStAbilityR1::OnBackground()
{
    HILOG_INFO("AmsStAbilityR1::OnBackground");
...
void AmsStAbilityR1::GetWantInfo(const Want &want)
{
    Want mWant(want);
...
std::string AmsStAbilityR1::Split(std::string &str, std::string delim)
{
    std::string result;
    if (!str.empty()) {
