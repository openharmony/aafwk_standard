...
#include "SetMissionStackSetting_fuzzer.h"
...
namespace OHOS {
    bool DoSomethingInterestingWithMyAPI(const uint8_t* data, size_t size)
    {
        ...
        // fuzz for StackSetting
        Parcel settingParcel;
        StackSetting *setting = nullptr;
        if (settingParcel.WriteBuffer(data, size)) {
            setting = StackSetting::Unmarshalling(settingParcel);
            if (setting) {
                abilityMgr->SetMissionStackSetting(*setting);
            }
        }
