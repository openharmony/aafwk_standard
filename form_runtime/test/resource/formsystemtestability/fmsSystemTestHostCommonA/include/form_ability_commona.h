...
#ifndef _FORM_ABILITY_COMMONA_H_
#define _FORM_ABILITY_COMMONA_H_
...
class FormEventSubscriberForCommonA;
class FormAbilityCommonA : public Ability {
public:
    ~FormAbilityCommonA();
    void SubscribeEvent();
    ...
    std::shared_ptr<FormEventSubscriberForCommonA> subscriber_;
...
protected:
    ...
    using FormFunc = void (FormAbilityCommonA::*)(std::string data);
    std::map<std::string, FormFunc> memberFuncMap_;
    std::shared_ptr<AcquireFormCallback> callback_;
};

class FormEventSubscriberForCommonA : public EventFwk::CommonEventSubscriber {
public:
    FormEventSubscriberForCommonA(const EventFwk::CommonEventSubscribeInfo &sp) : CommonEventSubscriber(sp)
    {
        ability_ = nullptr;
    }
    ~FormEventSubscriberForCommonA()
    {
        ability_ = nullptr;
    }
    ...
    FormAbilityCommonA *ability_;
};
}  // namespace AppExecFwk
}  // namespace OHOS
#endif  // _FORM_ABILITY_COMMONA_H_