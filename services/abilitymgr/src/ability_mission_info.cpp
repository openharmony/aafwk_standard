...
namespace OHOS {
namespace AAFwk {
bool AbilityMissionInfo::ReadFromParcel(Parcel &parcel)
{
    id = parcel.ReadInt32();
...
AbilityMissionInfo *AbilityMissionInfo::Unmarshalling(Parcel &parcel)
{
    AbilityMissionInfo *info = new (std::nothrow) AbilityMissionInfo();
    if (info == nullptr) {
        return nullptr;
    }
...
bool AbilityMissionInfo::Marshalling(Parcel &parcel) const
{
    parcel.WriteInt32(id);
    parcel.WriteInt32(runingState);
    parcel.WriteInt32(missionStackId);
