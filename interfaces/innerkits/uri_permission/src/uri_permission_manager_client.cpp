    ...
    const Security::AccessToken::AccessTokenID fromTokenId, const Security::AccessToken::AccessTokenID targetTokenId)
{
    ...
    ErrCode ret = ConnectUriPermService();
    if (ret != 0 || !uriPermMgr_) {
        HILOG_ERROR("Failed to ConnectUriPermService");
        return;
    }
    uriPermMgr_->GrantUriPermission(uri, flag, fromTokenId, targetTokenId);
}
    ...
    const Security::AccessToken::AccessTokenID tokenId)
{
    ...
    return uriPermMgr_->VerifyUriPermission(uri, flag, tokenId);
}
...
void UriPermissionManagerClient::RemoveUriPermission(const Security::AccessToken::AccessTokenID tokenId)
{
    ...
    uriPermMgr_->RemoveUriPermission(tokenId);
}

ErrCode UriPermissionManagerClient::ConnectUriPermService()
{
    ...
    sptr<UpmsDeathRecipient> recipient(new UpmsDeathRecipient(uriPermMgr_));
